...
import pandas as pd
from PDFS import Tree, calculate_pdfs

# Cấu hình giao diện
st.set_page_config(layout="wide", page_title="PDFS Visualizer")
st.markdown("<style>body { background-color: #111111; color: white; }</style>", unsafe_allow_html=True)

st.title("🌳 PDFS Algorithm Visualization")
...
# Run PDFS button
if st.sidebar.button("Run PDFS"):
    tree = Tree(n, root)
    for u, v, w in edges:
        tree.add_edge(u, v, w)

    routes = calculate_pdfs(tree, B)
...
if st.session_state.tree_built:
    ...
    # Layout tầng và root ở trên cùng
    net.set_options("""
    var options = {
      "layout": {
        "hierarchical": {
