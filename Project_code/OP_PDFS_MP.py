...
import pandas as pd
from OP_PDFS import Tree, calculate_pdfs

st.set_page_config(layout="wide", page_title="OP-PDFS Visualizer")
st.markdown("<style>body { background-color: #111111; color: white; }</style>", unsafe_allow_html=True)

st.title("🌳 OP-PDFS Algorithm Visualization")
...
# Run OP-PDFS button
if st.sidebar.button("Run OP-PDFS"):
    tree = Tree(n, root)
    for u, v, w in edges:
        tree.add_edge(u, v, w)
